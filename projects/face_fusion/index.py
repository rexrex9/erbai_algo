...
import traceback
from projects.face_fusion.major import Major
...
def _do(trace_id,media_path,face_path,out_format,ifoss):
    #异步存储数据库
    try:
        result_path = M.do(trace_id,media_path,face_path,out_format,ifoss)
...
def do():
    ...
    try:
        ...
        face_path = sf.getArgsMore('face_path')
        if face_path is None:raise Exception('face_path is None')
        temp_face_path = oss_base.download(face_path)

        out_format = sf.getArgsMore('out_format')
        ifoss = sf.getArgsMore('ifoss')
        trace_id = M.rq.init_progress(M.project_name)
        executor.submit(_do, trace_id,temp_path,temp_face_path,out_format,ifoss)
        r['ok'] = 1
        r['trace_id'] = trace_id
